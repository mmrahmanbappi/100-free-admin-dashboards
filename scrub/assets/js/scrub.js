/* Scrub - all behaviour. One namespace. Chart.js is the only dependency. */
(function (D) {
  "use strict";

  var S = window.SCRUB = {
    state: { range: "Today", compare: "Previous period", ward: "All wards" },
    charts: {}, live: null
  };
  ...
  S.metricValue = function (m) {
    switch (m.unit) {
      ...
      case "min": return S.int(m.value) + "<small> min</small>";
      case "days": return m.value.toFixed(1) + "<small> days</small>";
      default: return S.int(m.value);
    }
  };
  S.delta = function (v, invert, unit) {
    var good = invert ? v < 0 : v > 0;
    var suffix = unit === "abs" ? "" : unit === "pp" ? "pp" : "%";
    return {
      text: (v > 0 ? "+" : "") + (Math.abs(v) % 1 ? v.toFixed(1) : v) + suffix,
  ...
  S.toggleTheme = function () {
    ...
    try { localStorage.setItem("scrub.theme", next); } catch (e) {}
    S.rebuildCharts();
  ...
  S.toggleNav = function () {
    ...
    try { localStorage.setItem("scrub.min", on ? "1" : "0"); } catch (e) {}
    ...
    return merge({
      ...
      scales: {
        x: { grid: { display: false }, border: { display: false },
             ticks: { color: tick, font: { size: 11, family: "Roboto Mono" }, maxRotation: 0, autoSkipPadding: 26,
                      callback: label } },
        y: { beginAtZero: true, grid: { color: grid, drawTicks: false }, border: { display: false },
             ticks: { color: tick, font: { size: 11, family: "Roboto Mono" }, padding: 10, callback: label } }
      }
    }, opts || {});
  };
  function label(v) {
  ...
  function source(spec) {
    var b = spec.split(":");
    if (b[0] === "trace") {
      var v = D[b[1]].slice(), lab = [], t = 20 * 60;  /* ends at 20:00, half hourly */
      for (var i = v.length - 1; i >= 0; i--) {
        var mins = t - i * 30;
        lab.push(("0" + Math.floor(mins / 60)).slice(-2) + ":" + ("0" + (mins % 60)).slice(-2));
      }
      return { labels: lab, values: v, money: false };
    }
    if (b[0] === "metric") {
      var m = D.metrics[b[1]];
  ...
  var bandPlugin = {
    ...
    beforeDatasetsDraw: function (chart, args, opts) {
      var o = chart.options.clinical; if (!o) return;
      var ctx = chart.ctx, y = chart.scales.y, a = chart.chartArea;
      if (o.band) {
        ctx.save();
        ctx.fillStyle = token("--ok"); ctx.globalAlpha = 0.09;
        ctx.fillRect(a.left, y.getPixelForValue(o.band[1]), a.right - a.left,
                     y.getPixelForValue(o.band[0]) - y.getPixelForValue(o.band[1]));
        ctx.restore();
      }
      if (o.threshold !== undefined) {
        ctx.save();
        ctx.strokeStyle = token("--crit"); ctx.lineWidth = 1.4; ctx.setLineDash([5, 4]);
        ctx.beginPath();
        ctx.moveTo(a.left, y.getPixelForValue(o.threshold));
        ctx.lineTo(a.right, y.getPixelForValue(o.threshold));
        ctx.stroke();
        ctx.setLineDash([]);
        ctx.fillStyle = token("--crit");
        ctx.font = "11px 'Roboto Mono', monospace";
        ctx.fillText(o.label || "threshold", a.left + 6, y.getPixelForValue(o.threshold) - 6);
        ctx.restore();
      }
    }
  };
  ...
  S.initCharts = function () {
    document.querySelectorAll("canvas[data-chart]").forEach(function (cv, i) {
      ...
      var band = cv.getAttribute("data-band"), thr = cv.getAttribute("data-threshold");
      var thrLabel = cv.getAttribute("data-threshold-label");
      S.registerChart(cv.id, function (ctx) {
        ...
        if (kind === "donut") return new Chart(ctx, { type: "doughnut",
          ...
          options: { responsive: true, maintainAspectRatio: false, cutout: "66%",
            plugins: { legend: { position: "bottom", labels: { color: token("--ink-2"), boxWidth: 9,
              ...
              font: { size: 12, family: "Source Sans 3" } } } } } });
        ...
        if (band || thr) {
          base.clinical = {};
          if (band) base.clinical.band = band.split(",").map(Number);
          if (thr) { base.clinical.threshold = +thr; base.clinical.label = thrLabel || "escalation threshold"; }
          base.scales.y.beginAtZero = false;
          if (cv.getAttribute("data-min")) base.scales.y.min = +cv.getAttribute("data-min");
          if (cv.getAttribute("data-max")) base.scales.y.max = +cv.getAttribute("data-max");
        }
  ...
  R.kpis = function (el) {
    el.innerHTML = el.getAttribute("data-kpis").split(",").map(function (k, i) {
      k = k.trim();
      var m = D.metrics[k], d = S.delta(m.delta, m.invert, m.dunit || (m.unit === "pp" ? "pp" : "%"));
  ...
  R.feed = function (el) {
    el.innerHTML = D.feed.slice(0, 7).map(function (f) { return feedRow(f, f.t); }).join("");
  };
  function initials(s) {
  ...
  function feedRow(f, age) {
    return '<div class="feed-row">' + tile(f.who, initials(f.who), true) +
      '<span class="txt"><b>' + f.what + "</b><span>" + f.who + "</span></span>" +
      '<span class="t">' + (age || f.t) + "</span></div>";
  }
  S.feedRow = feedRow;
  R.funnel = function (el) {
    var max = D.pathway[0].value;
    el.innerHTML = D.pathway.map(function (s, i) {
  ...
  R.vitals = function (el) {
    el.innerHTML = D.vitals.slice(0, +(el.getAttribute("data-vitals") || 6)).map(function (v) {
      return '<div class="vital' + (v.state === "ok" ? "" : " is-" + v.state) + '">' +
        '<span class="k">' + v.key + "</span>" +
        '<span class="v">' + v.value + (v.unit ? "<u>" + v.unit + "</u>" : "") + "</span>" +
        '<span class="r">ref ' + v.ref + "</span>" +
        '<span class="flag">' + v.flag + "</span></div>";
    }).join("");
  };
  R.beds = function (el) {
    el.innerHTML = D.beds.map(function (b) {
      var free = b.state !== "Occupied";
      return '<div class="bed' + (free ? " is-free" : "") + (b.tone === "crit" ? " is-crit" : "") + '">' +
        "<b>" + b.bed + "</b>" +
        '<span class="who2">' + (b.patient || "Empty") + "</span>" +
        '<span class="pill' + (b.tone && b.tone !== "free" ? " pill--" + b.tone : "") + '"><i></i>' +
        b.state + "</span></div>";
    }).join("");
  };
  ...
  function tile(seed, text, sm) {
    ...
    return '<span class="av' + (sm ? " av--sm" : "") + '" style="background:var(--series-' +
      (n % 6 + 1) + ')">' + text + "</span>";
  }
  function cell(r, c) {
    var v = r[c.k];
    switch (c.f) {
      case "patient":
        var pn = v || r.name || r.patient || "";
        return '<a class="pt link" href="patient-detail.html">' +
          (r.acuity ? '<span class="acu acu-' + r.acuity + '" title="Acuity ' + r.acuity + '"></span>' : "") +
          tile(r.mrn || pn, r.initials || initials(pn)) +
          '<span class="pt-txt"><b>' + pn +
          (r.allergy ? '<span class="alg">' + r.allergy + "</span>" : "") + "</b>" +
          '<span class="mrn">' + (r.mrn ? "MRN " + r.mrn : r.location || "") + "</span></span></a>";
      case "link":
  ...
  S.setWard = function (v) {
    S.state.ward = v;
    document.querySelectorAll("[data-ward-label]").forEach(function (e) { e.textContent = v; });
    persist(); S.refresh();
  };
  function persist() {
    try { sessionStorage.setItem("scrub.state", JSON.stringify(S.state)); } catch (e) {}
  ...
  S.startLive = function () {
    ...
    S.live = setInterval(function () {
      ...
      if (feed) {
        var f = D.feed[Math.floor(Math.random() * D.feed.length)];
        feed.insertAdjacentHTML("afterbegin", feedRow(f, "now"));
  ...
  /* ============================== palette ============================= */
  var PAGES = [
    ["Ward overview","index.html"],["Patients","patients.html"],["Patient chart","patient-detail.html"],
    ["Beds","beds.html"],["Monitoring","monitoring.html"],["Admissions","admissions.html"],
    ["Discharges","discharges.html"],["Appointments","appointments.html"],["Clinic schedule","schedule.html"],
    ["Telehealth","telehealth.html"],["Vitals","vitals.html"],["Medication rounds","medications.html"],
    ["Prescriptions","prescriptions.html"],["Lab results","lab-results.html"],["Imaging","imaging.html"],
    ["Care plans","care-plans.html"],["Referrals","referrals.html"],["Patient activity","activity.html"],
    ["Rehab programmes","rehab.html"],["Records","records.html"],["Clinicians","staff.html"],
    ["Shift rota","rota.html"],["Tasks","tasks.html"],["Messages","messages.html"],
    ["Reports","reports.html"],["Department performance","department-report.html"],
    ["Billing","billing.html"],["Claims","claims.html"],["Supplies","supplies.html"],
    ["Equipment","equipment.html"],["Settings","settings.html"],["Components","ui-components.html"]
  ];
  ...
  function notifications() {
    ...
    host.innerHTML = '<div class="pop-head">Notifications</div>' +
      ...
      '<a class="btn btn--quiet btn--sm" href="monitoring.html">Open monitoring</a></div>';
  }
  ...
  S.init = function () {
    try {
      ...
      if (localStorage.getItem("scrub.min") === "1") document.body.classList.add("is-min");
      var st = sessionStorage.getItem("scrub.state");
    ...
    document.querySelectorAll("[data-vitals]").forEach(R.vitals);
    document.querySelectorAll("[data-beds]").forEach(R.beds);
    ...
    document.querySelectorAll("[data-ward-label]").forEach(function (e) { e.textContent = S.state.ward; });
    ...
    document.addEventListener("click", function (e) {
      ...
      if (act) {
        ...
        if (a === "ward") { S.setWard(v); closePops(); }
